...
#
#Wrong:
# expected: Image 'europe-west1-docker.pkg.dev/palladius-genai/ricc-hotrails-manhouse/ricc-hotrails-manhouse:latest' not found.
#  reality:        europe-west1-docker.pkg.dev/palladius-genai/ricc-hotrails/ricc-hotrails
#
...
if which gcloud ; then
  ...
  gcloud secrets versions access latest --secret=ricc-hotrails-envrc > .tmp.gcloud-envrc ||
...
if hostname | egrep 'ricc-macbookpro|derek|penguin' ; then
  ...
  export APP_NAME_TO_DEPLOY='ricc-hotrails-manhouse'
fi
...
#SECRET_REGION=$(gcloud secrets versions access latest --secret=ricc-hotrails_REGION)
...
#      --update-secrets=ricc-hotrails_SECRET_KEY=ricc-hotrails_SECRET_KEY:latest \
#      --service-account="ricc-hotrails-docker-runner@$PROJECT_ID.iam.gserviceaccount.com" \
# Useless:
